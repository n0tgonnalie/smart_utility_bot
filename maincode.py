...
r() #создаём диспетчера, он отвечает за обработку команд
...
async def command_start(message):
  await message.answer('Hello, выбери функцию с помощью которой я смогу помочь тебе, а инече... ничего не сделаю', reply_markup=test_kb())

@dp.message(Command('info'))
async def command_info(message):
  await message.answer('Я являюсь ботом для помощи в расчётах, учёбе, работе и повседневной жизни. В скорем времени мой функционал будет пополнятся!')
...
def link_kb():
      inline_kb_list = [
          [InlineKeyboardButton(text="график функции", url='https://www.desmos.com/calculator/s60mqvyp85?lang=ru'), InlineKeyboardButton(text="Калькулятор", url='https://www.desmos.com/scientific?lang=ru')]]
